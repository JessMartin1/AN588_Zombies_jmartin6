...
#AM: I believe this only works with tab separated data(txt, or tsv). Also this does not work on my computer because the path/location of the file is not the same and you're pulling it from your local storage. had to change this for it to work for me.
```
...
f <- read.csv("zombies.csv") #AM: This isn't working on my end. I had to change this to read variable "f"
...
print(mean_height)
(height_sd<-pop_sd(f$age, mean_height)) #AM: There is an error here, you're pulling on the age vector to computate height_sd
...
# Load the data from the CSV file
f <- read.csv("zombies.csv") #AM: I'm not familiar with the method of loading the file the way you did, but I dont believe assigning "f"  as the read csv s necesary for every new chunk
...
<br>AM: I coded the samae way but made it in one line because its easier for me to reat, but when I add colors I'll probably need to separate the line like you did here. I said that there was not relationship between age and weight because I did not really see a significatn trend between the two variables.
...
#AM: this doesn't work on my end I get the error "y is empty or has only NAs". I tried changing a few things but I'm not sure how to fix it for myself. Theoretically your code makes sense, I did it relatively the same way as you.
```
...
sd <- sd(s)
sd
sem <- sd(s)/sqrt(length(s)) #AM: Using sd finds the sample standard deviation, so you found the sample sd of the sample. Use you pop_sd function here to find the sd of this sample of 30
sem
...
ci <- c(lower, upper)
ci
#AM: I like this way of doing it. I enjoyed making functions so much I made an entire fucntion without using qqnorm to find ci :/
```
...
sd <- sd(s)
sd
sem <- sd(s)/sqrt(length(s)) #alternatively you can use sd since its a preset variable
sem
set <- NULL  # sets up a dummy variable to hold our 1000 simulations
n <- 30
for (i in 1:100) {
...
AM: You made different samples of each one, so none of the calculated data is related to each other. You only need to use the sample() function once, when you use it again you create a new sample of 30. Make a variable and set is to the sample fucntion(s <-sample(. . .) and use the variable "s" for the mean/sd/sem). I think he also wanted us to use the pop_sd function we made here as well. I realized I did this problem wrong and should use a different CI for the data that is not normally distributed.
...
## Peer Commentary:
A lot of our code is similar. I had issues with knitting your markdown document. It is tailored to your computer but not others. I also had difficulty with loading in you Q4 chunk for reasons that I could not fix. I also make minor typos in my code! I have a bad habit of rewriting code which I should fix as well. Your code helped me a lot in figuring out how to change my code for the better. I realized I misread some of the questions and have to redo them. I like your use of par(mfrow), I always think it's cool to use to show figures. I also love your aesthetics for the tables, very nice shades of blue. Love your picture!