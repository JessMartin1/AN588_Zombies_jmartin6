---
title: "AN588 Final Homework 3 (Fall 2023)" 
author: "Jess Martin"
date: "10/15/2023"
output: 
...
* AM: I believe your method of loading a file only works with tab separated data(txt, or tsv). Also, this does not work on my computer because the path/location of the file is not the same and you're pulling it from your local storage. I had to change this for it to work for me. JM: Yes, this is correct. Fixed! 

* AM: I also used a function, but I only place the vector as my input and calculated the mean inside the function to make it easier and more efficient. JM: Agreed, that would have been more efficient. I will keep that in mind for future assignments. 

* AM: There is an error here, you're pulling on the age vector to computer height_sd. JM: Good catch, fixed!

* AM: I love the colors and the decoration. Mine is a bit boring right now because I wanted to make sure I had it write before I added more code or else I would get overwhelmed. But other than that I coded the same way! JM: Thank you!

* AM: I'm not familiar with the method of loading the file the way you did, but I dont believe assigning "f <- read.csv(f)"  as the read csv s necesary for every new chunk(I deleted it because it was messing with my knit). JM: It was not necessary, fixed!

* AM: I coded the same way but made it in one line because its easier for me to repeat, but when I add colors I'll probably need to separate the line like you did here. I said that there was not relationship between age and weight because I did not really see a significant trend between the two variables. JM: I prefer the way it look not in one line, but I will keep this in mind for future assignments.

* AM: I agree with your conclusion! I did relatively the same thing, just without the for loop. JM: Yay, great!

* AM: I like the use of the for loop to plot everything. Very nice. JM: Thank you!

* AM: I like this way of doing it. I enjoyed making functions so much I made an entire function without using qqnorm to find ci, which was a lot of work :/ JM: Thank you!

* AM: You made different samples of each one, so none of the calculated data is related to each other. You only need to use the sample() function once, when you use it again you create a new sample of 30. Make a variable and set is to the sample function(s <-sample(. . .) and use the variable "s" for the mean/sd/sem). I think he also wanted us to use the pop_sd function we made here as well. I realized I did this problem wrong and should use a different CI for the data that is not normally distributed. JM: Fixed! Not sure if pop_sd had to be used here, thus I did not change it. 
...
## My Experience with Homework #3

**Three things that I found difficult about this assignment:**

1. I'm still not sure what the correct approach is for finding the CIs for variables that do not follow a normal distribution.
2. I struggled to add slope lines to my scatterplots for Question #3, until I found this [website](https://www.datanovia.com/en/blog/how-to-plot-a-smooth-line-using-ggplot2/).
3. I incorrectly loaded the data file and did not use the curl package. It will not happen again!

**Two things that I liked from my peer's comments:**

1. Very complimentary, it is always nice to receive positive feedback about your code.
2. Very informative, I learned so many things that I previously did not know. Also, their comments were exceptionally detailed, indicating the considerable thought and time were dedicated to writing them.

**One thing I think could have used improvement from my peer's comments:**

1. Nothing, I couldn't have asked for better comments.